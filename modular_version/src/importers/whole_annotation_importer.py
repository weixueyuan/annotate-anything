...
⚠️  已过时 - 建议使用通用导入器
推荐使用: python -m importers.generic_importer --task whole_annotation

使用方式：
    python -m importers.whole_annotation_importer                    # 导入默认文件
    python -m importers.whole_annotation_importer --source data.jsonl  # 导入指定文件
    python -m importers.whole_annotation_importer --clean            # 清空后导入
"""
...
class WholeAnnotationImporter:
    ...
    def transform_record(self, model_id: str, attrs: dict) -> dict:
        ...
        for key, value in attrs.items():
            ...
            # overall_description: 数组转字符串（UI显示需要）
            if key == 'overall_description' and isinstance(value, list):
                business_data[key] = '\n'.join(value)
            else:
                # 其他字段保持原样（包括 image_url）
...
def main():
    ...
    print("⚠️  注意：此导入器已过时")
    print("="*70)
    print("推荐使用通用导入器：python -m importers.generic_importer --task whole_annotation")
    print("通用导入器支持所有任务，功能更强大，维护更简单")
    print("="*70 + "\n")
    
    parser = argparse.ArgumentParser(description='导入整体物体标注数据')
    
    default_source = os.path.join(project_root, 'database_jsonl/whole_annotation.jsonl')
    default_db = os.path.join(project_root, 'databases/whole_annotation.db')
    
    parser.add_argument('--source', '-s', type=str, default=default_source,
                       help=f'数据源文件（默认: database_jsonl/whole_annotation.jsonl）')
    parser.add_argument('--db', '-d', type=str, default=default_db,
                       help=f'数据库路径（默认: databases/whole_annotation.db）')
    ...
    # 创建导入器并执行
    importer = WholeAnnotationImporter()
    ...
    print(f"✅ 可以运行: python src/main_multi.py --task whole_annotation --uid user1\n")
